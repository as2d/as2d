...
describe("miterLimit property", () => {
  it("should set the miterLimit value", () => {
    wasm.setMiterLimit(0.4);
    expect(wasm.getMiterLimit()).toBe(0.4);
  });

  var finiteTests: number[] = [Infinity, -Infinity, NaN];

  finiteTests.forEach((arg: number) => {
    it("should not set miterLimit value if set to " + arg, () => {
      wasm.setMiterLimit(arg);
      expect(wasm.getMiterLimit()).not.toBe(arg);
    });
  });
